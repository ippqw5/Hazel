...
void Level::OnRender()
{
	...
	float dim = m_Player.GetDim();
	//glm::vec4 color = Color::Green;
	// Background
	Renderer2D::DrawQuad({ playerPos.x, 0.0f, -0.8f }, { 50.0f, 50.0f }, dim, { 0.3f, 0.3f, 0.3f, 1.0f });
	//floor and ceiling
	Renderer2D::DrawQuad({ playerPos.x, 20.0f }, {50.0f, 10.0f}, dim, color);
	Renderer2D::DrawQuad({ playerPos.x, -20.0f }, {50.0f, 10.0f}, dim, color);

	for (auto& pillar : m_Pillars)
	{
		Renderer2D::DrawRotatedQuad(pillar.TopPosition, pillar.TopScale, glm::radians(180.0f), m_TriangleTexture, 1.0f, dim, color);
		Renderer2D::DrawRotatedQuad(pillar.BottomPosition, pillar.BottomScale, 0.0f, m_TriangleTexture, 1.0f, dim, color);
	}
...
void Level::CreatePillar(int index, float offset)
{
	...
	float gap = gap_base + Random::Float() * gap_base;
