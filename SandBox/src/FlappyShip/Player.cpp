...
{
	auto speedUpFactor = GetScore() / 5 ;
	m_Velocity.x = m_Velocity.x >= 15.0f ? 15.0f : 5.0f * (1.0f + speedUpFactor * 0.2f);
...
void Player::OnRender()
{
	...
	Renderer2D::DrawRotatedQuad({ m_Position.x, m_Position.y, 0.5f }, { 1.0f, 1.3f }, glm::radians(GetRotation()), m_ShipTexture, 1.0f, m_Dim);
}

void Player::OnImGuiRender()
{
	ImGui::SliderFloat("Engine Power", &m_EnginePower, 0.5f, 1.0f);
	ImGui::SliderFloat("Gravity", &m_Gravity, 0.4f, 1.0f);
	ImGui::SliderFloat("Dim", &m_Dim, 0.0f, 2.0f);
}
